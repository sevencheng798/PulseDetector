...
bool ZGReadPressTemp(uint8_t *PreTemData)
{
    ...
    /* read status */
     ...
     if(HAL_OK != ReceiveRegCommand(DTSReadAddr, STATUS_REG, 1, &command, 1))
        break;
...
#ifdef GD_SOFT_SIMULATION_IIC
    size = 5;
    if(HAL_OK != ReceiveRegCommand(DTSReadAddr, PRESS_REG1, 1, PreTemData, size))
      break;
#else 
      command = PRESS_REG1;
