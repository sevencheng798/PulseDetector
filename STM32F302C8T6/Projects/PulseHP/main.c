...
extern void StartCMDTask(void const * argument);
...
int main(void)
{
  ...
  osThreadDef(CMDTask, StartCMDTask, osPriorityAboveNormal, 0, 128);
  CMDTaskHandle = osThreadCreate(osThread(CMDTask), NULL);

  /* definition and creation of DROTask */
  osThreadDef(DROTask, StartDROTask, osPriorityNormal, 0, 128);
...
void StartDefaultTask(void const * argument)
{
  ...
  //MX_USB_DEVICE_Init();
  /* USER CODE BEGIN 5 */
  SendData[0] = 0xBB; //data packet header
  SendData[1] = 0x66;      
  SendData[2] = 0xBB;
  SendData[3] = 0x66;
  HAL_Delay(5*1000);
...
/* USER CODE BEGIN Header_StartDROTask */
/**
* @brief Function implementing the DROTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartDROTask */
void StartDROTask(void const * argument)
{
  /* USER CODE BEGIN StartDROTask */
  /* Infinite loop */
  //LED1_OFF();
  for(;;)
  {
    HPReadPress(&SendData[4]);
    CDC_Transmit_FS(SendData,14);
  }
  /* USER CODE END StartDROTask */
}
...
void probeSensorType(void) {
  ...
  uint8_t loop = 5;
  uint8_t probeData[16]={0xaa, 0xaa, 0x55, 0x55, 0x04, MCU_Probe_Sensor_Response_ACK, 0xfa, 0};
  uint8_t probeDataLengths = 9;
  
  do { 
    loop ++;
    if(HPReadPress(pRcv)) { // HP Sensor probe.
      // 
